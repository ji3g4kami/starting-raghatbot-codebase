...
class TestRAGSystemIntegration:
    ...
    def rag_system_with_mock_ai(self, integration_config):
        ...
        with patch('rag_system.AIGenerator') as mock_ai_gen:
            ...
            mock_ai_instance.generate_response.return_value = "This is a test response about testing concepts."
            
            rag_system = RAGSystem(integration_config)
            rag_system.mock_ai_generator = mock_ai_instance  # Store reference for test assertions
...
When testing integrations with external services, you often need to use mock services or stubs to simulate external dependencies and ensure predictable test outcomes."""
    ...
    def test_add_course_document_success(self, rag_system_with_mock_ai, sample_test_document):
        """Test successfully adding a course document"""
        # Execute
        course, chunk_count = rag_system_with_mock_ai.add_course_document(sample_test_document)
    ...
    def test_add_course_document_file_not_found(self, rag_system_with_mock_ai):
        ...
        # Execute
        course, chunk_count = rag_system_with_mock_ai.add_course_document("/nonexistent/file.txt")
        
        # Assert
        assert course is None
        assert chunk_count == 0
    
    def test_query_with_successful_search(self, rag_system_with_mock_ai, sample_test_document):
        ...
        # Execute
        response, sources = rag_system_with_mock_ai.query("What is integration testing?")
        
        # Assert
        assert response == "Integration testing involves verifying that different components work together."
    ...
    def test_query_with_session_management(self, rag_system_with_mock_ai, sample_test_document):
        ...
        # First query
        response1, sources1 = rag_system_with_mock_ai.query("What is integration testing?", session_id)
    ...
    def test_query_without_session_creates_session(self, rag_system_with_mock_ai, sample_test_document):
...
Content for getting started."""
        ...
        # Execute
        total_courses, total_chunks = rag_system_with_mock_ai.add_course_folder(str(tmp_path))
    ...
    def test_add_course_folder_nonexistent(self, rag_system_with_mock_ai):
        ...
        # Execute
        total_courses, total_chunks = rag_system_with_mock_ai.add_course_folder("/nonexistent/folder")
        
        # Assert
        assert total_courses == 0
        assert total_chunks == 0
    
    def test_add_course_folder_clear_existing(self, rag_system_with_mock_ai, tmp_path, sample_test_document):
        ...
        new_doc.write_text("""Course Title: New Course
Course Instructor: New Instructor
Lesson 0: New Content
This is new content.""")
        
        # Execute with clear_existing=True
        total_courses, total_chunks = rag_system_with_mock_ai.add_course_folder(str(tmp_path), clear_existing=True)
...
Some content here."""
    ...
    def test_query_error_propagation(self, integration_config):
        ...
        with patch('rag_system.AIGenerator') as mock_ai_gen:
            ...
            mock_request = Mock()
            mock_body = {}
            mock_ai_instance.generate_response.side_effect = anthropic.APIError("Invalid API key", request=mock_request, body=mock_body)
    ...
    def test_empty_vector_store_query(self, rag_system_with_mock_ai):
        ...
        mock_ai = rag_system_with_mock_ai.mock_ai_generator
        mock_ai.generate_response.return_value = "I don't have any information about that topic."
    ...
    @pytest.mark.parametrize("query,expected_tool_call", [
        ("What is integration testing?", True),  # Content question - should use search
        ("Hello", False),  # General greeting - might not use tools
        ("List all courses", True),  # Might use outline tool
    ])
    def test_tool_usage_based_on_query_type(self, rag_system_with_mock_ai, sample_test_document, query, expected_tool_call):
    ...
    def test_source_retrieval_and_reset(self, rag_system_with_mock_ai, sample_test_document):
        """Test that sources are properly retrieved and reset between queries"""
        # Setup
        rag_system_with_mock_ai.add_course_document(sample_test_document)
        
        # Mock the tool manager methods
        with patch.object(rag_system_with_mock_ai.tool_manager, 'get_last_sources') as mock_get_sources, \
             patch.object(rag_system_with_mock_ai.tool_manager, 'reset_sources') as mock_reset_sources:
            
            mock_get_sources.return_value = [
                {"text": "Integration Testing Course - Lesson 1", "url": "https://example.com/lesson1"}
            ]
            
            # First query
            response1, sources1 = rag_system_with_mock_ai.query("What is integration testing?")
